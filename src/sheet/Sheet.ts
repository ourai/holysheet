...
import AbstractTable, { getColumnTitle, getColumnIndex } from '../table';
...
class Sheet extends AbstractTable implements ISheet {
  ...
  private selection: SheetSelection | null = null;

  private merged: Record<string, SheetRange> = {};

  private getInternalRowsInRange(): InternalSheetRow[] {
  ...
  public fill(cells: CellData[]): void {
    ...
    cells.forEach(cell => {
      ...
      if (colSpan > 0 || rowSpan > 0) {
        ...
        (this.cells[id] as InternalSheetCell).mergedCoord = mergedCoord;
  ...
  public getSelection(): SheetSelection | null {
    return this.selection;
  }

  public setSelection(selection: SheetSelection): void {
  ...
  public getRowsInRange(): SheetRow[] {
    return this.getTableRows(this.getInternalRowsInRange());
  }

  public getCellsInRange(): SheetCell[] {
    if (!this.selection) {
      return [];
    }

    const cellsInRange: SheetCell[] = [];
  ...
  public getModifiedCellsInRange(): SheetCell[] {
  ...
  public mergeCells(): Result {
    ...
    rowsInRange.forEach((row, ri) => {
      ...
      if (ri === 0) {
        ...
        const cell = this.cells[cellId] as InternalSheetCell;
  ...
  public unmergeCells(): Result {
    ...
    const rowsInRange = this.getInternalRowsInRange();
    const rows: InternalSheetRow[] = this.createRows(
      startRowIndex,
      rowsInRange.length,
      this.getColumnCount(),
      true,
    ...
    rowsInRange.forEach((row, ri) => {
      ...
      row.cells.forEach(cellId => {
        const { span, mergedCoord, ...pureCell } = this.cells[cellId] as InternalSheetCell;
      ...
      this.rows.forEach((row, ri) => {
        ...
        if (hitCells.length > 0) {
          for (let idx = hitCells.length - 1; idx >= 0; idx--) {
            ...
            } = this.cells[cellId] as InternalSheetCell;
            const [colSpan = 0, rowSpan = 0] = span;
            ...
            if (colSpan > 0 && (colIndex < startColIndex || calcColIndex > endColIndex)) {
              if (colIndex < startColIndex) {
                ...
                if (newColSpan === 0 && rowSpan === 0) {
                  ...
                  delete (this.cells[cellId] as InternalSheetCell).mergedCoord;
                } else {
                  ...
                  (this.cells[cellId] as InternalSheetCell).mergedCoord = newMergedCoord;
                ...
                if (newColSpan > 0 || rowSpan > 0) {
                  ...
                  (this.cells[newCellId] as InternalSheetCell).mergedCoord = newMergedCoord;
        ...
        row.cells.forEach(cellId => {
          ...
          if (colIndex > endColIndex) {
            ...
            if (mergedCoord) {
              ...
              const range: SheetRange = [newColIndex, sri, newColIndex + (span[0] || 0), eri];
              const newMergedCoord = getTitleCoord(...range);
  ...
  public deleteRows(startRowIndex: number, count?: number): Result {
    ...
    if (overflowCells.length > 0) {
      ...
      overflowCells.forEach(cellId => {
        ...
        if (colSpan > 0 || newRowSpan > 0) {
          ...
          (this.cells[newCellId] as InternalSheetCell).mergedCoord = mergedCoord;
    ...
    this.rows.splice(startRowIndex, resolvedCount).forEach(row => {
      row.cells.forEach(cellId => {
        const { mergedCoord } = this.cells[cellId] as InternalSheetCell;
    ...
    if (this.getRowCount() > 0) {
      if (startRowIndex > 0) {
        ...
        this.rows.slice(baseRowindex, startRowIndex).forEach((row, ri) => {
          row.cells.forEach(cellId => {
            ...
            delete this.merged[(this.cells[cellId] as InternalSheetCell).mergedCoord!];
            ...
            if (colSpan === 0 && newRowSpan === 0) {
              ...
              const [sci, sri, eci] = getIndexCoord(
                (this.cells[cellId] as InternalSheetCell).mergedCoord!,
              );
              const range: SheetRange = [sci, sri, eci, sri + newRowSpan];
              const mergedCoord = getTitleCoord(...range);

              (this.cells[cellId] as InternalSheetCell).mergedCoord = mergedCoord;
      ...
      this.rows.slice(startRowIndex, this.getRowCount()).forEach((row, ri) => {
        row.cells.forEach(cellId => {
          const { mergedCoord, span = [] } = this.cells[cellId] as InternalSheetCell;
          ...
          if (mergedCoord) {
            ...
            const range: SheetRange = [sci, newRowIndex, eci, newRowIndex + (span[1] || 0)];
            const newMergedCoord = getTitleCoord(...range);
